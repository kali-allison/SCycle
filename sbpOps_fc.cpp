...
switch ( order ) {
    case 2:
    {
      ...
      D1(0,0,-1.0*scale);D1int(0,1,scale); // first row
      for (Ii=1;Ii<N-1;Ii++) {
  ...
  _Hz(Nz,Nz),_D1z(Nz,Nz),_D1zint(Nz,Nz),_Iz(Nz,Nz)
{
  ...
  // going from 1D in y to 2D:
  {
    ...
    sbpSpmat_fc(order,Ny,1.0/dy,_Hy,Hyinv,_D1y,_D1yint,Sy);
  ...
  // going from 1D in z to 2D:
  {
    ...
    if (Nz > 1) { sbpSpmat_fc(order,Nz,1/dz,_Hz,Hzinv,_D1z,_D1zint,Sz); }
    else { _Hz.eye(); }
