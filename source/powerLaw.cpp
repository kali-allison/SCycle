...
PetscErrorCode DislocationCreep::loadSettings()
{
  ...
  while (getline(infile, line))
  {
    ...
    if (var.compare("AVals")==0) { loadVectorFromInputFile(rhsFull,_AVals); }
...
PetscErrorCode PowerLaw::setMMSInitialConditions(const PetscScalar time)
{
  ...
  ierr = computeViscStrainSourceTerms(viscSource,_gVxy,_gVxz);CHKERRQ(ierr);
  ...
  Vec& gVxy_t, Vec& gVxz_t)
{
  ...
  VecPointwiseMult(gVxy_t,_mu,SAT);
...
PetscErrorCode PowerLaw::writeStep1D(PetscInt stepCount, const string outputDir)
{
  PetscErrorCode ierr = 0;
  #if VERBOSE > 1
...
PetscErrorCode PowerLaw::writeStep2D(PetscInt stepCount, const string outputDir)
{
  PetscErrorCode ierr = 0;
  #if VERBOSE > 1
