                 ...
                 it_vec dvarBegin,it_vec dvarEnd)
{
  ...
  VecCopy(*(varBegin+4),_T); // uncomment to uncouple
                 ...
                 it_vec dvarBegin,it_vec dvarEnd)
{
  ...
  mapToVec(source,MMS_pl_gxz_t_source,_Nz,_dy,_dz,time);
...
PetscErrorCode PowerLaw::setTempRate(const PetscScalar time,const_it_vec varBegin,const_it_vec varEnd,
                                          it_vec dvarBegin,it_vec dvarEnd)
{
  PetscErrorCode ierr = 0;
  #if VERBOSE > 1
    string funcName = "PowerLaw::setTempRate";
    ierr = PetscPrintf(PETSC_COMM_WORLD,"Starting %s in %s: time=%.15e\n",funcName.c_str(),FILENAME,time);
    CHKERRQ(ierr);
  #endif
  ...
  VecSet(bcB,1643.0); // temp at depth
  ...
  ierr = VecAXPY(_T,1.0,rhs);CHKERRQ(ierr);
