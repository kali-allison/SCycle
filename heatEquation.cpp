...
PetscErrorCode HeatEquation::constructMapV()
{
  ...
  if (_wVals.size() > 0 ) { ierr = setVecFromVectors(_w,_wVals,_wDepths); CHKERRQ(ierr); }
  else { VecSet(_w,0.); }
...
PetscErrorCode HeatEquation::computeHeatFlux()
{
  ...
  getTemp(totalTemp);
  ierr = _sbp->muxDz(totalTemp,_heatFlux); CHKERRQ(ierr);
...
PetscErrorCode HeatEquation::writeStep2D(const PetscInt stepCount, const PetscScalar time,const std::string outputDir)
{
  ...
  if (stepCount == 0) {
    ierr = io_initiateWriteAppend(_viewers, "Tamb", _Tamb, outputDir + "Tamb"); CHKERRQ(ierr);
