...
PetscErrorCode SbpOps_fc_coordTrans::getAlphay(PetscScalar &alphaDy)
{
  #if VERBOSE > 2
    string funcName = "SbpOps_fc_coordTrans::getAlphay";
    string fileName = "SbpOps_fc_coordTrans.cpp";
    PetscPrintf(PETSC_COMM_WORLD,"Starting %s in %s\n",funcName.c_str(),fileName.c_str());
  #endif
...
PetscErrorCode SbpOps_fc_coordTrans::getAlphaz(PetscScalar &alphaDz)
{
  #if VERBOSE > 2
    string funcName = "SbpOps_fc_coordTrans::getalphaDz";
    string fileName = "SbpOps_fc_coordTrans.cpp";
    PetscPrintf(PETSC_COMM_WORLD,"Starting %s in %s\n",funcName.c_str(),fileName.c_str());
  #endif
...
PetscErrorCode SbpOps_fc_coordTrans::getMus(Mat &mu,Mat &muqy,Mat &murz) { mu = _mu; muqy = _muqy; murz = _murz; return 0; }
PetscErrorCode SbpOps_fc_coordTrans::getR(Mat& Ry, Mat& Rz) { Ry=_Ry; Rz=_Rz; return 0; }
...
PetscErrorCode SbpOps_fc_coordTrans::getBs(Mat& By_Iz,Mat& Iy_Bz)
{
  By_Iz = _By_Iz;
  Iy_Bz = _Iy_Bz;
  return 0;
...
{ // enclose in brackets so that destructors are called at end
  ...
  if (!_bcLType.compare("Dirichlet")) {
    MatDestroy(&_rhsL);
    ierr = MatMatMatMult(tempMats._muqy,tempMats._Hyinv_Iz,e0y_Iz,
            MAT_INITIAL_MATRIX,PETSC_DEFAULT,&_rhsL);CHKERRQ(ierr);
    ...
    ierr = MatMatMatMult(tempMats._muqy,tempMats._Hyinv_Iz,E0y_Iz,
             MAT_INITIAL_MATRIX,PETSC_DEFAULT,&tempMats._AL);CHKERRQ(ierr);
  ...
  if (!_bcRType.compare("Dirichlet")) {
    ...
    MatDestroy(&_rhsR);
    ierr = MatMatMatMult(tempMats._muqy,tempMats._Hyinv_Iz,eNy_Iz,
            MAT_INITIAL_MATRIX,PETSC_DEFAULT,&_rhsR);CHKERRQ(ierr);
    ...
    ierr = MatMatMatMult(tempMats._muqy,tempMats._Hyinv_Iz,ENy_Iz,
             MAT_INITIAL_MATRIX,PETSC_DEFAULT,&tempMats._AR);CHKERRQ(ierr);
  ...
  // These matrices are nnz if Nz > 1
{
  ...
  if (!_bcTType.compare("Dirichlet")) {
    ...
    MatDestroy(&_rhsT);
    ierr = MatMatMatMult(tempMats._murz,tempMats._Iy_Hzinv,Iy_e0z,
            MAT_INITIAL_MATRIX,PETSC_DEFAULT,&_rhsT);CHKERRQ(ierr);
    ...
    // if bcR = displacement: _alphaD*mu*Iy_Hzinv*Iy_E0z + _beta*Iy_Hzinv*_muxIy_BSzT*Iy_E0z
    ierr = MatMatMatMult(tempMats._murz,tempMats._Iy_Hzinv,Iy_E0z,
             MAT_INITIAL_MATRIX,PETSC_DEFAULT,&tempMats._AT);CHKERRQ(ierr);
  ...
  if (!_bcBType.compare("Dirichlet")) {
    ...
    MatDestroy(&_rhsB);
    ierr = MatMatMatMult(tempMats._murz,tempMats._Iy_Hzinv,Iy_eNz,
...
PetscErrorCode SbpOps_fc_coordTrans::constructRzmu(const TempMats_fc_coordTrans& tempMats,Mat &Rzmu)
{
  ...
  MatMult(tempMats._rz,*_muVec,murzV);
...
switch ( _order ) {
    case 2:
    {
      ...
      MatDestroy(&Iy_D2zT);
      ierr = MatMatMatMult(temp,tempMats._murz,Iy_D2z,MAT_INITIAL_MATRIX,PETSC_DEFAULT,&Rzmu);CHKERRQ(ierr);
    ...
    case 4:
    {
      ...
      Mat mu3;
      {
        MatDuplicate(tempMats._murz,MAT_COPY_VALUES,&mu3);
...
PetscErrorCode SbpOps_fc_coordTrans::constructRymu(const TempMats_fc_coordTrans& tempMats,Mat &Rymu)
{
  ...
  MatMult(tempMats._qy,*_muVec,muqyV);
...
switch ( _order ) {
    case 2:
    {
      ...
      MatDestroy(&D2y_IzT);
      ierr = MatMatMatMult(temp,tempMats._muqy,D2y_Iz,MAT_INITIAL_MATRIX,PETSC_DEFAULT,&Rymu);CHKERRQ(ierr);
    ...
    case 4:
    {
      ...
      Mat mu3;
      {
        MatDuplicate(tempMats._muqy,MAT_COPY_VALUES,&mu3);
        PetscScalar mu=0;
        PetscInt Ii,Jj,Istart,Iend=0;
      ...
      MatDestroy(&D4y_IzT);
      ierr = MatMatMatMult(temp1,tempMats._muqy,D4y_Iz,MAT_INITIAL_MATRIX,PETSC_DEFAULT,&Rymu);CHKERRQ(ierr);
  ...
  _muqy(NULL),_murz(NULL),_yq(NULL),_zr(NULL)
{
  ...
  PetscObjectSetName((PetscObject) _Dy_Iz, "_Dy_Iz");
  kronConvert(_Iy,_D1z,_Iy_Dz,5,5);

  PetscObjectSetName((PetscObject) _Iy_Dz, "_Iy_Dz");
  computeJacobian(mu);

  MatMatMult(mu,_qy,MAT_INITIAL_MATRIX,1.0,&_muqy);
  MatMatMult(mu,_rz,MAT_INITIAL_MATRIX,1.0,&_murz);
  ...
  // going from 1D in y to 2D:
  {
    ...
    // mu*kron(BSy,Iz)
    {
      ...
      MatMatMult(_muqy,temp,MAT_INITIAL_MATRIX,1.0,&_muxBSy_Iz);
  ...
  // going from 1D in z to 2D:
  {
    ...
    // mu*kron(Iy,BSz)
    {
      ...
      MatMatMult(_murz,temp,MAT_INITIAL_MATRIX,1.0,&_muxIy_BSz);
...
PetscErrorCode TempMats_fc_coordTrans::computeJacobian(Mat& mu)
{
  ...
  MatDuplicate(mu,MAT_DO_NOT_COPY_VALUES,&_yq);
  MatDuplicate(mu,MAT_DO_NOT_COPY_VALUES,&_zr);
