...
PetscErrorCode computeSlipVel(UserContext * D)
{
  ...
  for (Ii=Istart;Ii<Iend;Ii++) {
    ...
    if (leftVal==rightVal) { outVal = leftVal; }
      ...
      ierr = bisect((*frictionLaw),Ii,leftVal,rightVal,&outVal,D->rootTol,1e3,D);CHKERRQ(ierr);
    }
...
PetscErrorCode timeStepRHSFuncPETSC(TS ts, PetscReal t, Vec in,Vec out, void * ctx)
{
...
#if VERBOSE > 1
  ierr = PetscPrintf(PETSC_COMM_WORLD,"Starting timeStepRHSFunc in timeStepping.c, at time t=%g\n",t);
  CHKERRQ(ierr);
#endif

  // update uhat and psi from input w
  ierr = VecGetOwnershipRange(in,&Istart,&Iend); // was geting from D->w
  for (Ii=Istart;Ii<Iend;Ii++) {
    if ( (Ii<D->Nz) & (Ii<Iend) ) {
      ierr = VecGetValues(in,1,&Ii,&wVal);CHKERRQ(ierr);
      ierr = VecSetValue(D->uhat,Ii,wVal,INSERT_VALUES);CHKERRQ(ierr);
    }
    if ( (Ii>=D->Nz) & (Ii<Iend) ) {
      ierr = VecGetValues(in,1,&Ii,&wVal);CHKERRQ(ierr);
      ierr = VecSetValue(D->psi,Ii-D->Nz,wVal,INSERT_VALUES);CHKERRQ(ierr);
    }
  }
  ierr = VecAssemblyBegin(D->uhat);CHKERRQ(ierr);ierr = VecAssemblyBegin(D->psi);CHKERRQ(ierr);
  ierr = VecAssemblyEnd(D->uhat);CHKERRQ(ierr);ierr = VecAssemblyEnd(D->psi);CHKERRQ(ierr);

  // update gF from input w
  ierr = VecGetOwnershipRange(in,&Istart,&Iend);
  for (Ii=Istart;Ii<Iend;Ii++) {
    if ( Ii<D->Nz) {
      ierr = VecGetValues(in,1,&Ii,&wVal);CHKERRQ(ierr);
      ierr = VecSetValue(D->gF,Ii,wVal/2.0,INSERT_VALUES);CHKERRQ(ierr);
    }
  }
  ierr = VecAssemblyBegin(D->gF);CHKERRQ(ierr);
  ierr = VecAssemblyEnd(D->gF);CHKERRQ(ierr);
  ...
  for (Ii=Istart;Ii<Iend;Ii++) {
    if ( Ii<D->Nz) {
      wVal = D->vp*t/2;
      ierr = VecSetValue(D->gR,Ii,wVal,INSERT_VALUES);CHKERRQ(ierr);
    }
  }
  ierr = VecAssemblyBegin(D->gR);CHKERRQ(ierr);
  ierr = VecAssemblyEnd(D->gR);CHKERRQ(ierr);
  ...
  // create dw from vel and dpsi
  ierr = VecGetOwnershipRange(D->V,&Istart,&Iend);
  for (Ii=Istart;Ii<Iend;Ii++) {
      ierr = VecGetValues(D->V,1,&Ii,&wVal);CHKERRQ(ierr);
      ierr = VecSetValue(out,Ii,wVal,INSERT_VALUES);CHKERRQ(ierr);

      ierr = agingLaw(Ii,&wVal,D);CHKERRQ(ierr);
      ierr = VecSetValue(out,Ii+D->Nz,wVal,INSERT_VALUES);CHKERRQ(ierr);
  }
  ierr = VecAssemblyBegin(out);CHKERRQ(ierr);
  ierr = VecAssemblyEnd(out);CHKERRQ(ierr);

#if VERBOSE > 1
  ierr = PetscPrintf(PETSC_COMM_WORLD,"Ending timeStepRHSFunc in timeStepping.c, at time t=%g\n",t);CHKERRQ(ierr);
#endif
...
PetscErrorCode timeMonitorPETSC(TS ts,PetscInt step,PetscReal time,Vec w,void *ctx)
{
  ...
  if ( step % D->strideLength == 0) {
    D->count++;
    D->currTime = time;
    ierr = D->writeCurrentStep();CHKERRQ(ierr);
    ierr = PetscPrintf(PETSC_COMM_WORLD,"time = %g,count=%i\n",time,D->count);CHKERRQ(ierr);
  }
