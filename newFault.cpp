...
PetscErrorCode NewFault::setFields(Domain& D)
{
  ...
  if (_stateLaw.compare("flashHeating") == 0) {
    ...
    VecDuplicate(_tauQSP,&_c);  PetscObjectSetName((PetscObject) _c, "c_fault");
  }
...
PetscErrorCode NewFault_qd::computeVel()
{
  ...
  PetscScalar *etaA, *tauQSA, *sNA, *psiA, *aA,*bA,*slipVelA;
...
: _a(a),_b(b),_sN(sN),_tauQS(tauQS),_eta(eta),_psi(psi),_N(N),_v0(v0)
{ }
...
PetscScalar agingLaw_psi(const PetscScalar& psi, const PetscScalar& slipVel, const PetscScalar& b, const PetscScalar& f0, const PetscScalar& v0, const PetscScalar& Dc)
{
  ...
  if ( isinf(dstate) || isnan(dstate) ){
    dstate = 0;
  }
  assert(!isnan(dstate));
  assert(!isinf(dstate));
  return (PetscScalar)dstate;
}
