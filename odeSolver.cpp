...
PetscErrorCode FEuler::integrate_switch(IntegratorContextEx *obj)
{
#if VERBOSE > 1
  PetscPrintf(PETSC_COMM_WORLD,"Starting FEuler::integrate in odeSolver.cpp.\n");
#endif
  ...
  // set initial condition
  ierr = obj->d_dt_qd(_currT,_var,_dvar);CHKERRQ(ierr);
  ierr = obj->timeMonitor_qd(_currT,_stepCount,_var,_dvar,stopIntegration);CHKERRQ(ierr); // write first step
  ...
  while (_stepCount<_maxNumSteps && _currT<_finalT) {
    ...
    if (_currT>_finalT) { _currT = _finalT; }
    _stepCount++;
    ierr = obj->timeMonitor_qd(_currT,_stepCount,_var,_dvar,stopIntegration);CHKERRQ(ierr);
  }
...
PetscErrorCode RK32::integrate_switch(IntegratorContextEx *obj)
{
#if VERBOSE > 1
  PetscPrintf(PETSC_COMM_WORLD,"Starting RK32::integrate in odeSolver.cpp.\n");
#endif
  ...
  while (_stepCount<_maxNumSteps && _currT<_finalT) {
    ...
    while (attemptCount<100) { // repeat until time step is acceptable
      ...
      ierr = obj->d_dt_qd(_currT+0.5*_deltaT,_varHalfdT,_dvarHalfdT);CHKERRQ(ierr);
      ...
      ierr = obj->d_dt_qd(_currT+_deltaT,_vardT,_dvardT);CHKERRQ(ierr);
...
PetscErrorCode RK43::integrate_switch(IntegratorContextEx *obj)
{
#if VERBOSE > 1
  PetscPrintf(PETSC_COMM_WORLD,"Starting RK43::integrate in odeSolver.cpp.\n");
#endif
